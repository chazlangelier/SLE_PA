...
This file uses better graphic settings than 20221208_sc_DE_testing_lupus_only_physact.Rmd

# Import packages

```{r}
wd <- "/Users/hoangvanphan/Library/CloudStorage/Box-Box/Van_research/20220715_CLUES_lupus"
setwd(paste0(wd,"/analysis"))
...
# library(lme4) # for mixed effects model, don't need to load it explicitly
# library(MAST)
library(edgeR)
# library(Seurat)
# library(muscat)
...
# Set number of cores for MAST
# Using > 3 cores would take up too much RAM
options("mc.cores"=3)
...
for (i in c("inactive_t1","inactive_t3","biologicalsex","race")) {
...
```{r}
# Missing old steroid info
print(sum(is.na(metadata$steroral3mohigh)))

# Missing new steroid info
print(sum(is.na(metadata$steroral3mohigh.new)))
```


```{r}
# GSEA pathways
go.bp <- msigdbr::msigdbr(species = "Homo sapiens", category = "C5", subcategory = "GO:BP")
go.bp.fgsea <- go.bp %>% split(x = .$gene_symbol, f = .$gs_name)
...
reactome.fgsea <- reactome %>% split(x = .$gene_symbol, f = .$gs_name)
reactome.cp <- reactome %>% select(gs_name, gene_symbol) # for clusterProfiler
```
...
for (i in c("T4","T8","B","cDC","cM","ncM","NK","pDC")) { # c("T4","T8","B","cDC","cM","ncM","NK","pDC","Prolif","PB","Progen")
  ...
  # print(hist(lrt$table$PValue, main=i, xlab="PValue"))
...
for (i in c("T4","T8","B","cM")) {
  write.csv(
    subset(inactive.up[[i]], FDR < 0.1),
    sprintf("%s/gene_list/%s/%s_edgeR-LRT_inactive-t3-up_age-biologicalsex-race-covariate_fdr0.1.csv", wd, subfolder, i)
  )
...
for (i in c("T4","T8","B","cDC","cM","ncM","NK")) { # c("T4","T8","B","cDC","cM","ncM","NK","pDC","Prolif","PB","Progen")
...
# Get list of pathways that are significant in at least one analysis
my.pathways <- union(
  hallmark.gsea.res[["T4"]][hallmark.gsea.res[["T4"]]$padj<0.1,]$pathway,
  hallmark.gsea.res[["T8"]][hallmark.gsea.res[["T8"]]$padj<0.1,]$pathway
)
# Filter for these overlapping pathways
merged.hallmark.gsea <- merged.hallmark.gsea %>%
...
ggplot(data=merged.hallmark.gsea) +
  geom_point(mapping=aes(x=reorder(gsub("^HALLMARK_","",pathway),NES), y=NES, fill=as.factor(NES>0),
                         colour=as.factor(p.adjust < 0.1)),
             pch=21, size=2, stroke=1) +
  geom_hline(yintercept=0, color="black") +
  scale_color_manual(values=c("white","black")) +
  scale_y_continuous(breaks=seq(-4,4,2), limits=c(-3,3)) +
  labs(x="Hallmark pathways", y="NES", 
       title="Pathways upregulated in\ninactive patients") +
  coord_flip() + facet_wrap(~ cell.type) +
  theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5, size=12, face="plain"),
    axis.text = element_text(size=12, color="black"),
...
ggsave(
  paste0(wd,"figures/",subfolder,"/physfunt_GSEA_T4_T8.svg"),
  width = 10, height=7, units="in"
)
# ggsave(
#   paste0(wd,"figures/",subfolder,"/GSEA_T4_T8.png"),
#   width = 10, height=7, units="in"
# )
```

## Covariates: age, sex, race (patients with steroid info only)
...
Plot IPA upstream analysis results. I will use the IPA results from 20221208_sc_DE_testing_lupus_only_physact, because the DE analysis is the same as this script.
