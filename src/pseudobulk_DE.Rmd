...
analyze the physical activity and stress level of the patients. I will control
for age, sex and race in all analyses. Some analyses will have additional covariates.

# Import packages

```{r}
wd <- "/Users/hoangvanphan/Library/CloudStorage/Box-Box/Van_research/20220715_CLUES_lupus/SLE_PA"
...
# Import metadata
metadata <- read.csv(
  file.path(wd, "data", "metadata.csv")
)

for (i in c("inactive","biologicalsex","race")) {
...
steroid <- read.csv(
  file.path(wd, "data", "steroids.csv")
)
steroid$subjectid <- as.character(steroid$subjectid)

# Merge
metadata <- merge(
  metadata, steroid[,c("subjectid","steroids")],
...
print(sum(is.na(metadata$steroids)))
```
...
for (i in c("T4","T8","B","cDC","cM","ncM","NK","pDC")) { # c("T4","T8","B","cDC","cM","ncM","NK","pDC","Prolif","PB","Progen")
  ...
  pseudobulk <- read.csv(
    file.path(wd, "pseudobulk_counts",
              sprintf("Ye_lab_annotated_cellxgene_lupus_only_physact_%s_pseudobulk_count.csv", i)),
  ...
  # Get metadata of samples that are present in pseudobulk, and have a valid inactive
  metadata.temp <- metadata %>%
    subset(!is.na(inactive) & (subjectid %in% colnames(pseudobulk))) %>%
    droplevels()
  ...
  y <- calcNormFactors(y)
  design <- model.matrix(~ inactive + age + biologicalsex + race, metadata.temp)
  y <- estimateDisp(y, design)
  fit <- glmFit(y, design)
  lrt <- glmLRT(fit, coef="inactive1")
...
ggsave(
  file.path(wd, "out", "volcano_plot_T4-T8.svg"),
...
boxplot(
  my.cpm[["T4"]]["IRF2BP2",] ~ metadata[match(colnames(my.cpm[["T4"]]),metadata$subjectid),"inactive"]
)
```
...
for (i in c("T4","T8","B","cM")) {
  write.csv(
    inactive.up[[i]],
    file.path(wd, "out",
              sprintf("%s_edgeR-LRT_inactive-up_age-biologicalsex-race-covariate_fdr1.0.csv", i)
    )
  )
}
```
...
for (i in names(hallmark.gsea.res)) {
  write.csv(
    hallmark.gsea.res[[i]],
    file.path(wd, "out",
              sprintf("%s_hallmark-GSEA-clusterProfiler_inactive-up_age-biologicalsex-race-covariate_fdr1.0.csv", i)
    )
  )
}
```
...
ggsave(
  file.path(wd, "out", "hallmark-gsea_T4-T8.svg"),
  plot = p,
...
ggsave(
  file.path(wd, "out", "hallmark-gsea_cM-B.svg"),
  plot = p[[1]]+p[[2]] + plot_layout(nrow=1),
...
p2 <- enrichplot::cnetplot(
  ...
  scale_colour_gradient2(low="#67a9cf", high="#ef8a62", midpoint = 0)
ggsave(
  file.path(wd, "out", "hallmark-gsea_cnetplot_T4.svg"),
  plot = p2,
...
write.csv(
  cnetplot.data,
  file.path(wd, "out", "hallmark-gsea_cnetplot_T4_data.csv")
)
```
...
for (i in c("T4","T8","B","cM")) { # c("T4","T8","B","cDC","cM","ncM","NK","pDC","Prolif","PB","Progen")
  ...
  y <- DGEList(counts = pseudobulk.temp)
  y <- calcNormFactors(y)
  design <- model.matrix(~ inactive + age + biologicalsex + race + physfunt, metadata.temp)
  y <- estimateDisp(y, design)
  fit <- glmFit(y, design)
  lrt <- glmLRT(fit, coef="inactive1")
...
for (i in c("T4","T8")) {
  write.csv(
    inactive.up[[i]],
    file.path(wd, "out",
              sprintf("%s_edgeR-LRT_inactive-up_age-biologicalsex-race-physfunt-covariate_fdr1.0.csv", i))
  )
}
```
...
for (i in names(hallmark.gsea.res)) {
  write.csv(
    hallmark.gsea.res[[i]],
    file.path(wd, "out",
              sprintf("%s_hallmark-GSEA-clusterProfiler_inactive-up_age-biologicalsex-race-physfunt-covariate_fdr1.0.csv", i))
  )
}
```
...
ggsave(
  file.path(wd, "out", "hallmark-gsea_physfunt_T4-T8.svg"),
  plot=p,
...
for (i in c("T4","T8","B","cDC","cM","ncM","NK","pDC")) { # c("T4","T8","B","cDC","cM","ncM","NK","pDC","Prolif","PB","Progen")
  ...
  # Get metadata of samples that are present in pseudobulk, and have a valid inactive and valid steroids
  metadata.temp <- metadata %>%
    subset(!is.na(inactive) & (subjectid %in% colnames(pseudobulk)) &
             !is.na(steroids)) %>%
    droplevels()
  ...
  y <- DGEList(counts = pseudobulk.temp)
  y <- calcNormFactors(y)
  design <- model.matrix(~ inactive + age + biologicalsex + race + as.factor(steroids), metadata.temp)
  y <- estimateDisp(y, design)
  fit <- glmFit(y, design)
  lrt <- glmLRT(fit, coef="inactive1")
...
ggsave(
  file.path(wd, "out", "hallmark-gsea_steroid_T4-T8.svg"),
  plot = p,
...
ipa.T4 <- readxl::read_excel(
  file.path(wd, "data", "T4_edgeR-LRT_inactive-up_age-biologicalsex-race-covariate_fdr1.0_cytokine-z2.0-fdr0.1.xls"),
 skip = 1
)
# Reformat column names
...
ggsave(
  file.path(wd, "out", "T4_IPA-cytokine.svg"),
  width = 2.7, height = 4, units = "in"
)

# T8
ipa.T8 <- readxl::read_excel(
  file.path(wd, "data", "T8_edgeR-LRT_inactive-up_age-biologicalsex-race-covariate_fdr1.0_cytokine-z2.0-fdr0.1.xls"),
 skip = 1
)
# Reformat column names
...
ggsave(
  file.path(wd, "out", "T8_IPA-cytokine.svg"),
